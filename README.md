...
### Skills Graph
- `GET /skills/graph/related?skill={skill}` - Get related skills (prerequisites and successors)
- `GET /skills/graph/prereqs?skill={skill}` - Get direct prerequisites
- `GET /skills/graph/path?skill={skill}` - Get learning path
- `GET /skills/all` - Get all available skills
- `GET /skills/graph/stats` - Get graph statistics
- `POST /skills/add` - Add a new skill
- `POST /skills/add-prerequisite` - Add prerequisite relationship
...
- `POST /ai/roadmap` - Generate learning roadmap
  ...
  ```
- `POST /ai/explain-skill-gaps` - Explain skill gaps
  ```json
  {
    "user_skills": ["Python"],
...
- `GET /test/system` - System status test

## 🎯 Usage

### Skill Graph Explorer
1. Navigate to `/skill-graph`
2. Type a skill name (e.g., "Python", "React", "Machine Learning")
3. Choose a view:
   - **Prerequisites**: Skills needed before this one
   - **Related Skills**: Prerequisites and successors
   - **Learning Path**: Ordered path to acquire the skill

### AI Roadmap Generator
1. Navigate to `/roadmap`
2. Add skills you want to learn
3. Click "Generate Roadmap"
4. Get a detailed 4-week learning plan with:
   - Weekly objectives
   - Learning resources
   - Practice projects
...
### Backend Port
Default: `8001`. Change in startup command or update frontend `.env.local`.

### Skills Graph
The graph is automatically seeded on startup with:
- Programming languages (Python, JavaScript, Java, etc.)
- Web development skills (React, Node.js, TypeScript, etc.)
- Data Science & ML (Pandas, Machine Learning, Deep Learning, etc.)
- Backend & DevOps (Docker, Kubernetes, AWS, etc.)
